...
Проект по мозданию ML-модели для прогнозирования оттока клиентов телекоммуникационной компании.
...
--port 5000
```
4. В другом командном окне запустить основной пайплайн:
```
...
Основная идея отбора - избежание переобучения модели и улучшение качества модели. Также с помощью отбора можно повысить интерпретируемость признаков и снизить экономические траты для сбора данных в будущем (потребуется потратить меньше сил и ресурсов для сбора данных по малому набору признаков) 
...
После фильтрации признаков тренировочный датасет имеет размерность: `(4225, 38)`. В датасете нет дублирующих строк, и у двух признаков есть пропущенные значения NaN, а именно у Тип интернета и Принятое маркетинговое предложение. Так как тип интернета действительно может отсутствовать (например, клиент не пользуется интернетом) и предложение тоже могло быть не предложено клиенту, то значения NaN было трансформировано в 0, как отдельную категорию. 
...
Таким образом, SHAP позволил интерпретировать чёрный ящик модели и объяснить бизнесу причины предскаханий модели. Полученные результаты подтверждают значимость поведенческих и сервисных факторов в задаче прогнозирования оттока клиентов. 
...
Перед построением модели передаётся список категориальных признаков напрямую в модель, используются автоматический баланс веса классов и механизм ранней остановки обучения.
...
В файле `notebooks/ML without_SS.ipynb` построены модели без признака `Satisfaction Score`. Данный признак показал доминирующую важность и сильно объясняет фактор оттока клиента. В данном датасета возможен риск скрытой утечки данных, но методы сбора данных в датасеты неизвестны. 

### Итоговые результаты савнения моделей:

| Модель | Recall | Precision | f1 | accuracy |
|---|-------------|-----------|----|----------|
| Logistic Regression | 0.65 | 0.71| 0.68 | 0.84 |
| Random Forest | 0.68 | 0.61| 0.65 | 0.80 |
| Catboost | 0.73 | 0.58| 0.65 | 0.79 |

Значение Recall=0.7 близко к реальным значениям. 

Важность признаков для CatBoost выглядит следующим образом:
...
Для автоматизации процессов сборки и проверки проекта был реализован CI/CD-конвейер с использованием платформы GitHub и инструмента GitHub Actions. GitHub используется для хранения исходного кода и управления версиями, а GitHub Actions - для автоматизации тестирования и сборки образа Docker.

В рамках CI-конвейера выполняется запуск тестов, разработанных с использованием фреймворка Pytest, которые были описаны выше. После успешного прохождения тестов GitHub Actions выполняет автоматическую сборку Docker-образа проекта на основе файла Dockerfile.

## Схема работы CI/CD-конвейера

В рамках проекта процесс работы конвейера можно представить следующим образом:
```
Разработка кода
        ↓
...
Сборка Docker-образа
        ↓
Готовый артефакт для запуска
```

В разделе Actions можно увидеть статус о пройдённых этапах:
![alt text](images/image-4.png)

## Команды git

Команды для отправки изменений на удалённый репозиторий:
```
git add .
...
Мониторинг качества ML-модели и инфраструктуры реализован с помощью платформы MlFlow. Необходимо запустить mlflow-сервер, который будет отображать собранные метрики. Все метрики собираются в один эксперимент, в рамках которого можно анализировать все обученные модели.

Так как основной моделью для обучения была выбрана CatBoost, то дополнительно в основной pipeline были добавлены модели логистической регрессии и случайного леса. Выбор модели осуществляется с помощью переменной `MODEL_NAME` в файле `.env`. Обучив все модели по-очереди, в веб-интерфейсе mlflow по адресу `http://127.0.0.1:5000` можно наблюдать динамику обучений.

## Мониторинг качества модели
Одной из задач мониторинга является контроль качества обучаемых моделей.

В рамках проекта в MLflow автоматически сохраняются:
- название модели
- гиперпараметры модели 
- метрики качества модели - время обучения, recall, accuracy, f1
- системные метрики (CPU, диски, сеть, память)
- артефакты обучения (топ-10 признаков по важности, матрица ошибок)
- артефакты модели

В веб-интерфейсе можно увидеть параметры выбранной модели:
![alt text](images/image-5.png)

А также сравнительный анализ по всем моделям:
...
Использование MLflow позволяет сравнивать модели, анализировать завимость между гиперпараметрами и метриками, отслеживать изменения качества между экспериментами. Это позволяет сохранять последовательность проводимых экспериментов и воспроизвести необходимую версию модели. 

## Мониторинг инфраструктуры

Для контроля использования вычислительных ресурсов был реализован мониторинг инфраструктуры средствами MLflow System Metrics. Во время выполнения экспериментов автоматически собираются системные показатели среды выполнения такие как:
- загрузка процессора (CPU)
- использование оперативной памяти (RAM)
- нагрузка на вычислительные ресурсы
- потребление ресурсов процессом обучения модели

![alt text](image-4.png)

Стоит отметить, что метрики собираются, когда изменения есть. При обучении моделей линейной регрессии или случайного леса метрики могут не отображаться, так как они не несут тяжёлую нагрузку на вычислительные ресурсы. Обычно большая нагрузка есть при подборе гиперпараметров и при использовании тяжёлых моделей в промышленных средах.

## Результаты мониторинга

В результате мониторинга была отображена картина сравнения моделей. Модель CatBoost на несколько долей секунд обучается дольше и среди метрик лидирует только по метрике Recall. На вычислительные ресурсы нагрузка есть, но незначительная. Таким образом, реализованный мониторинг позволяет удобно отслеживать динамику метрик, следить за историей обучений моделей, следить за нагрузкой на инфраструктуру и делать необходимые выводы по использованию той или иной модели. 
